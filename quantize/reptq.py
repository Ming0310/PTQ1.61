...
from models.int_llama_layer_reptq import QuantLlamaDecoderLayer
    ...
    for i in range(len(layers)):
        ...
        for name,module in qlayer.named_modules():
            if isinstance(module, QuantLinear):
                ...
                if args.quant_type == "mix":
                    mask = torch.load(os.path.join(args.output_dir, f"mask/{args.quant_type}_{args.wbits}_{i}_{name_tmp}_mask.pth"))
                else:
                    mask = act >= thresh
        ...
        if args.epochs > 0:
            ...
            optimizer = torch.optim.AdamW(
                # [{"params":qlayer.scaling_parameters(),"lr":9e-5},{"params":qlayer.bias_parameters(),"lr":9e-5},{"params":qlayer.rotation_parameters(),"lr":9e-3}],weight_decay=args.wd) #llama: 5e-4 1e-3 opt:llama2:9e-5 9e-3
            ...
            for epochs in range(args.epochs):
                ...
                for j in range(args.nsamples//args.batch_size):
                    ...
                    with traincast():
                        qlayer.binary_temporary(args.wbits, dev, args.quant_type) # 量化和smooth 权重 得到的权重为反量化后的权重
